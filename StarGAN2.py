...
picpath= "Stargan2/pic"
modelpath="Stargan2/models"

def load(name):
    with open(name,"rb") as f:
        return pickle.load(f)

def gen_resBlock(filter,strides=1):
    def f(x):
        shape=x.shape
        shortcut=x
        x = layers.Conv2D(filter,kernel_size=3,strides=strides,padding="same")(x)
        x = tfa.layers.InstanceNormalization()(x)
        x = layers.ReLU()(x)
        x = layers.Conv2D(filter, kernel_size=3,strides=1, padding="same")(x)
        x = tfa.layers.InstanceNormalization()(x)

        if filter!=shape[3] or strides!=1:
            shortcut=layers.Conv2D(filter,kernel_size=1,strides=strides,padding="same")(shortcut)
        return layers.Add()([x,shortcut])
    return f
def dis_resBlock(filter,strides=1):
    def f(x):
        shape=x.shape
        shortcut=x
        x = layers.Conv2D(filter,kernel_size=3,strides=strides,padding="same")(x)
...
def upSampling(filter,kernel_size=5,img_kernel=5):
    def f(x):
        x = layers.UpSampling2D((2,2))(x)
        x = layers.Conv2D(filter,kernel_size=kernel_size,padding="same")(x)
        x = tfa.layers.InstanceNormalization()(x)
        x = layers.ReLU()(x)

        img_out = layers.Conv2D(3,kernel_size=img_kernel,padding="same",activation="tanh")(x)
        return x,img_out
    return f
def upSamplings(count):
    def f(x):
        filter=x.shape[3]
        img_out=layers.Conv2D(3,kernel_size=5,padding="same",activation="tanh")(x)
        for i in range(count):
            filter=filter//2
            x,temp_img=upSampling(filter)(x)
            img_out=layers.UpSampling2D()(img_out)
            img_out=layers.Add()([img_out,temp_img])
        return img_out
    return f
def build_generator(img_shape):
    ...
    for i in range(2):
        filter*=2
        x = downSampling(filter)(x)
    for i in range(6):
        x = gen_resBlock(filter)(x)
    img_out=upSamplings(2)(x)
    img_out=layers.Lambda(lambda x:x*1/3.0)(img_out)
    model=Model([inp_img,inp_c],img_out,name="generator")
    return model
...
def build_discriminator(img_shape):
    ...
    x=layers.Conv2D(filter,kernel_size=7,activation="relu",padding="same")(discriminator_inp)

    for i in range(repeat):
        x=dis_resBlock(filter,strides=2)(x)
        filter*=2

    src_out=layers.Conv2D(1,kernel_size=3,padding="same",use_bias=False)(x)
    #src_out=layers.Flatten()(src_out)

    cls_out=layers.Conv2D(3,kernel_size=x.shape[1],padding="valid",use_bias=False)(x)
...
def build_generator_updatefunction(generator,discriminator):
    ...
    loss=-src_loss+cls_loss+10*rec_loss+5*id_loss
...
if __name__ == '__main__':
    ...
    try:
        train(generator,discriminator,img,cls,epochs=10000,save_span=100,version=version,update_weight=10)
